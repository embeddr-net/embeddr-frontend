...
export async function uploadImage(file: File, metadata?: any): Promise<PromptImage> {
...
export async function fetchTags(): Promise<Array<{ name: string; count: number }>> {
  ...
  if (!response.ok) {
    console.warn(`[API] Tags endpoint unavailable: ${response.status} ${response.statusText}`);
    return [];
  }
  return response.json();
}
