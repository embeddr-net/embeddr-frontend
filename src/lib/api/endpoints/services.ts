...
export interface ServiceOverview {
  type: string;
  adapters: Array<ServiceOverviewAdapter>;
}

export async function fetchServices(): Promise<Array<ServiceOverview>> {
  const res = await fetchWithAuth(`${BACKEND_URL}/services`);
  if (!res.ok) throw new Error("Failed to fetch services");
  return res.json();
}

export async function fetchAdapters(serviceType: string): Promise<Array<ServiceAdapter>> {
...
export async function setAdapterDefault(serviceType: string, name: string): Promise<void> {
  const res = await fetchWithAuth(
    `${BACKEND_URL}/services/${serviceType}/adapters/${name}/default`,
    {
      method: "POST",
    },
  );
