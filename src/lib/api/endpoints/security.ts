...
export const fetchSecurityOverviewStatus = async (): Promise<SecurityOverviewStatus> => {
  const res = await fetchWithAuth(`${BACKEND_URL}/security/overview`);
  if (!res.ok) {
...
export const fetchSecurityOperatorProfile = async (): Promise<SecurityOperator> => {
...
export const updateSecurityUser = async (payload: {
  ...
  const res = await fetchWithAuth(`${BACKEND_URL}/security/users/${payload.user_id}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
...
export const revokeSecurityKey = async (keyId: string): Promise<{ ok: boolean }> => {
...
export const updateSecurityRole = async (payload: {
  ...
  const res = await fetchWithAuth(`${BACKEND_URL}/security/roles/${payload.role_id}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      description: payload.description ?? null,
...
export async function fetchAuthorizationInfo(params: {
  ...
  const res = await fetchWithAuth(`${BACKEND_URL}/security/auth/authorize?${qs.toString()}`);
  if (res.status === 401) {
...
export async function submitAuthorizationConsent(body: {
  ...
  const res = await fetchWithAuth(`${BACKEND_URL}/security/auth/authorize/consent`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
