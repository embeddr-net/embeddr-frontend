...
export interface ResourceAdaptersResponse {
  adapters: Array<ResourceAdapterInfo>;
}
...
class EmbeddrApi {
  ...
  public auth = {
    setSession: async (options?: { apiKey?: string | null; clear?: boolean }) => {
  ...
  public artifacts = {
    list: (params: {
      ...
      return this.request<PaginatedResponse<Artifact>>(`/artifacts/?${q.toString()}`);
    },
    ...
    get: (id: string, input?: { include_owner_profiles?: boolean }) => {
      ...
      return this.request<Artifact>(`/artifacts/${id}${q.toString() ? `?${q.toString()}` : ""}`);
    },

    getEmbeddings: (id: string) =>
      this.request<Array<ArtifactEmbedding>>(`/artifacts/${id}/embeddings`),

    getAnnotations: (id: string) =>
      this.request<Array<ArtifactAnnotation>>(`/artifacts/${id}/annotations`),

    getLineage: (id: string) => this.request<LineageResponse>(`/artifacts/${id}/lineage`),

    getRelations: (id: string) =>
      this.request<Array<ArtifactRelation>>(`/artifacts/${id}/relations`),

    getProvenance: (id: string) => this.request<ProvenanceResponse>(`/artifacts/${id}/provenance`),
    ...
    getSubgraph: (
      ...
      const q = new URLSearchParams();
      if (params.maxDepth !== undefined) q.append("max_depth", `${params.maxDepth}`);
    ...
    getGraphTaxonomy: () => this.request<ArtifactGraphTaxonomy>(`/artifacts/graph/taxonomy`),

    getContentUrl: (id: string) => this.withApiKey(`${this.baseUrl}/artifacts/${id}/content`),

    getPreviewUrl: (id: string, type: "thumbnail" | "preview" = "thumbnail") =>
      this.withApiKey(`${this.baseUrl}/artifacts/${id}/preview?preview_type=${type}`),
    ...
    delete: (id: string) => this.request(`/artifacts/${id}`, { method: "DELETE" }),
    ...
    search: (query: string, limit: number = 20, offset: number = 0, typeName?: string) => {
      const q = new URLSearchParams();
      q.append("q", query);
      q.append("limit", limit.toString());
      q.append("offset", offset.toString());
      if (typeName) q.append("type_name", typeName);
      return this.request<PaginatedResponse<Artifact>>(`/artifacts/search?${q.toString()}`);
    },

    semanticSearchCap: (query: string, limit = 20, model?: string, space?: string) => {
      const q = new URLSearchParams();
      q.append("q", query);
      q.append("limit", limit.toString());
    ...
    findSimilarCap: (artifactId: string, limit = 20, model?: string, space?: string) => {
      const q = new URLSearchParams();
      q.append("limit", limit.toString());
      if (model) q.append("model", model);
      if (space) q.append("space", space);
      return this.request(`/artifacts/${artifactId}/similar/cap?${q.toString()}`);
    },
    ...
    findSimilar: (artifactId: string, limit: number = 20, model?: string, space?: string) => {
      const q = new URLSearchParams();
      q.append("limit", limit.toString());
      if (model) q.append("model", model);
      if (space) q.append("space", space);
  ...
  public resources = {
    listAdapters: () => this.request<ResourceAdaptersResponse>(`/resources/adapters`),
  ...
  public system = {
    ...
    listAutomations: () => this.request<AutomationListResponse>(`/system/automation/list`),
    ...
    getIngestionPipeline: () => this.request<IngestionPipelineConfig>(`/system/ingestion/pipeline`),
    ...
    applyIngestionDefaults: (input?: { enabled_plugins?: Array<string> }) =>
    ...
    getBlobRegistry: () => this.request<BlobRegistryResponse>(`/system/blob-registry`),
    getArtifactRegistry: () => this.request<ArtifactRegistryResponse>(`/system/artifact-registry`),
    getRelationOntology: () => this.request<RelationOntologyResponse>(`/system/relation-ontology`),
  ...
  public lotus = {
    query: (q: string, limit = 20) =>
      this.request<{ query: string; results: Array<any> }>(
    ...
    list: (params?: {
      ...
      const qs = q.toString();
      return this.request<LotusCapabilityListResponse>(`/lotus/list${qs ? `?${qs}` : ""}`);
    },
    ...
    dispatch: (result_id: string, kind: "action" | "nav" | "feature", data: any) =>
  ...
  public executions = {
    ...
    list: (params?: {
      ...
      if (params?.q) q.append("q", params.q);
      const qs = q.toString();
      return this.request<PaginatedResponse<Execution>>(`/executions${qs ? `?${qs}` : ""}`);
    },
    get: (id: string) => this.request<Execution>(`/executions/${id}`),
    activeCount: () => this.request<{ active: number }>(`/executions/active-count`),
    wait: (id: string, params?: { timeout_s?: number; poll_interval_s?: number }) => {
      const q = new URLSearchParams();
      if (params?.timeout_s) q.append("timeout_s", params.timeout_s.toString());
      if (params?.poll_interval_s) q.append("poll_interval_s", params.poll_interval_s.toString());
      const qs = q.toString();
      return this.request<Execution>(`/executions/${id}/wait${qs ? `?${qs}` : ""}`);
    },
    ...
    artifacts: (id: string, action?: string) => {
      ...
      const qs = q.toString();
      return this.request<Array<ExecutionArtifactLink>>(
        `/executions/${id}/artifacts${qs ? `?${qs}` : ""}`,
      );
    },
    children: (id: string) => this.request<Array<Execution>>(`/executions/${id}/children`),
  };

  public plugins = {
    list: () => this.request<Array<any>>("/plugins"),
    listLogos: async () => {
      const data = await this.request<{ logos: Record<string, string | null> }>("/plugins/logos");
  ...
  public collections = {
    list: (category?: "library" | "source") =>
      this.request<Array<CollectionResponse>>(
        `/collections${category ? `?category=${category}` : ""}`,
      ),
    listScanners: () => this.request<Array<ScannerTypeInfo>>(`/collections/scanners`),
    ...
    update: (id: string, input: { label?: string; scanner_config?: Record<string, any> }) =>
    ...
    rescan: (id: string) => this.request(`/collections/${id}/scan`, { method: "POST" }),
    remove: (id: string) => this.request(`/collections/${id}`, { method: "DELETE" }),
  };
  ...
  public maintenance = {
    ...
    fixTypes: (limit: number = 1000) =>
      this.request<MaintenanceFixTypesResponse>(`/maintenance/fix-types?limit=${limit}`),
    listScripts: () => this.request<Array<MaintenanceScript>>(`/maintenance/scripts`),
...
async function fetchActions(): Promise<Array<PluginAction>> {
...
async function fetchExecutions(params?: {
  ...
  limit?: number;
  offset?: number;
}): Promise<Array<ArtifactExecution>> {
...
async function fetchArtifacts(params?: {
  ...
  type_name?: string;
  q?: string;
}): Promise<{ items: Array<Artifact>; total: number }> {
  const endpoint = params?.q ? `${BACKEND_URL}/artifacts/search` : `${BACKEND_URL}/artifacts/`;
