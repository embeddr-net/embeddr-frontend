      ...
      loadExternalPlugins: async (options) => {
        ...
        try {
          const ensureRegistrySync = () => {
            ...
            (window as any).__embeddrRegistrySync = usePluginRegistry.subscribe((state) => {
              set({
...
export const extendApiForPlugin = (api: EmbeddrAPI, pluginId: string): EmbeddrAPI => {
  if (!api || !api.utils) {
    console.error("[PluginStore] extendApiForPlugin called with invalid api:", api);
    return api;
  }
    ...
    plugin: {
      ...
      request: async (path: string, init?: RequestInit) => {
        ...
        if (!res.ok) {
          ...
          throw new Error(
            errorJson?.detail || errorJson?.error || errorText || `Request failed: ${res.status}`,
          );
        }
        return res.json();
      },
...
export const useEmbeddrAPI = (): EmbeddrAPI => {
  ...
  const events = useMemo(
    () => ({
      ...
      emit: (event: any, ...args: Array<any>) => globalEventBus.emit(event, ...args),
    }),
    [],
  );
  ...
  const utils = useMemo(() => {
    ...
    return {
      ...
      getPanels,
      subscribePanelState: (listener: (panels: Array<any>) => void) => {
        const store = useWindowStore;
        const notify = () => listener(getPanels());
        notify();
        return store.subscribe(notify);
      },
      uploadImage: async (file: File, prompt?: string, parent_ids?: Array<string | number>) => {
  ...
  const artifacts = useMemo(() => {
    const list = async (input: {
      ...
      const res = await fetchWithAuth(`${BACKEND_URL}/artifacts/?${q.toString()}`);
      if (!res.ok) {
    ...
    const resolve = async (input: { id: string; variant?: "preview" | "original" }) => {
    ...
    const create = async (input: {
      ...
      const res = await fetchWithAuth(`${BACKEND_URL}/lotus/embeddr-core.artifact.create`, {
        method: "POST",
    ...
    const uploadInit = async (input: {
      ...
      const res = await fetchWithAuth(`${BACKEND_URL}/lotus/embeddr-core.artifact.upload.init`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(input),
      });
    ...
    const uploadComplete = async (input: { upload_id: string; confirm?: boolean }) => {
      const res = await fetchWithAuth(
    ...
    const get = async (id: string, input?: { include_owner_profiles?: boolean }) => {
      const q = new URLSearchParams();
    ...
    const getRelations = async (id: string) => {
      const res = await fetchWithAuth(`${BACKEND_URL}/artifacts/${id}/relations`);
      if (!res.ok) {
        const txt = await res.text();
    ...
    const addRelation = async (
      ...
      const res = await fetchWithAuth(`${BACKEND_URL}/artifacts/${sourceId}/relations`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    const resolve = async (input: {
      ...
      if (artifactId && !url) {
        ...
        return {
          id: artifactId,
          type: hintType || "image",
          content_url: appendAuth(`${BACKEND_URL}/artifacts/${artifactId}/content`),
          preview_url: appendAuth(`${BACKEND_URL}/artifacts/${artifactId}/preview`),
        };
      }
  ...
  const settings = useMemo(
    () => ({
      ...
      set: (key: string, value: any) => useSettingsStore.getState().setSetting(key, value),
      getPlugin: <T = any>(pluginId: string, key: string, defaultValue?: T) =>
        useSettingsStore.getState().getPluginSetting(pluginId, key, defaultValue),
    ...
    () => ({
      list: async (input: { category: string; page?: number; limit?: number }) => {
        ...
        try {
          ...
          if (!res.ok) return { items: [], total: 0, page, limit, pages: 0, category };
  ...
  return useMemo(() => {
    const api = {
      ...
      ui: {
        ...
        isPanelActive: (panelId: string) => usePanelStore.getState().activePanelId === panelId,
      },
      workspaces: {
        getState: () => useWorkspaceStore.getState(),
        subscribe: (listener: (state: any) => void) => useWorkspaceStore.subscribe(listener),
        list: () => useWorkspaceStore.getState().listWorkspaces(),
        getActiveId: () => useWorkspaceStore.getState().activeWorkspaceId,
        ensureDefault: () => useWorkspaceStore.getState().ensureDefaultWorkspace(),
        create: (name: string, options?: { fromCurrent?: boolean; isTemplate?: boolean }) =>
        ...
        apply: (id: string) => useWorkspaceStore.getState().setActiveWorkspace(id),
        rename: (id: string, name: string) =>
          useWorkspaceStore.getState().renameWorkspace(id, name),
        clone: (id: string, name?: string) => useWorkspaceStore.getState().cloneWorkspace(id, name),
        remove: (id: string) => useWorkspaceStore.getState().deleteWorkspace(id),
      ...
      windows: {
        open: (id: string, title: string, componentId: string, props?: any) => {
          useWindowStore.getState().openWindow({ id, title, componentId, props });
        },
        ...
        spawn: (componentId: string, title: string, props?: any) => {
          ...
          if (!resolveWindowComponent(componentId)) {
            setTimeout(() => {
              ...
              const stillExists = Boolean(useWindowStore.getState().windows[id]);
              if (!stillExists) return;
              console.warn(`[windows.spawn] component not registered: ${componentId}`);
              toastApi.warning(
      ...
      executions: {
        ...
        get: async (executionId: string) => {
          const res = await fetchWithAuth(`${BACKEND_URL}/executions/${executionId}`);
          if (!res.ok) {
            const txt = await res.text();
        ...
        list: async (input?: {
          ...
          const qs = params.toString();
          const res = await fetchWithAuth(`${BACKEND_URL}/executions${qs ? `?${qs}` : ""}`);
          if (!res.ok) {
            const txt = await res.text();
        ...
        cancel: async (executionId: string) => {
          const res = await fetchWithAuth(`${BACKEND_URL}/executions/${executionId}/cancel`, {
            method: "POST",
          });
          if (!res.ok) {
            const txt = await res.text();
        ...
        nudge: async (
          ...
          const res = await fetchWithAuth(`${BACKEND_URL}/executions/${executionId}/nudge`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
      ...
      lotus: {
        ...
        query: async (q: string, limit = 20) => {
          ...
          const res = await fetchWithAuth(`${BACKEND_URL}/lotus/query?${params.toString()}`);
          if (!res.ok) {
            const txt = await res.text();
        ...
        list: async (input?: {
          ...
          const qs = params.toString();
          const res = await fetchWithAuth(`${BACKEND_URL}/lotus/list${qs ? `?${qs}` : ""}`);
          if (!res.ok) {
            const txt = await res.text();
      ...
      plugins: {
        ...
        getComponents: (location: string) => usePluginStore.getState().getComponents(location),
        getActions: (location: string) => usePluginStore.getState().getActions(location),
        getApi: (pluginId: string) => extendApiForPlugin(api, pluginId),
      },
      security,
      artifacts,
      resources,
