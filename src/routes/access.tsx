...
import { fetchSecurityOverviewStatus, loginWithPassword } from "@/lib/api/endpoints/security";
import { embeddrApi } from "@/lib/api/client";
import { useUserStore } from "@/store/userStore";

const AccessPage = () => {
  const navigate = useNavigate();
  const { apiKey, setApiKey, setDisplayName, setAvatarUrl, setIsOperator } = useUserStore();
  const [status, setStatus] = useState<"checking" | "ok" | "required" | "error">("checking");
    ...
    try {
      ...
      if (result.ok) {
        if (result.data?.current_user) {
          setDisplayName(
            result.data.current_user.display_name || result.data.current_user.username,
          );
    ...
    <div className="flex h-full min-h-0 items-center justify-center p-6">
      <Card className="w-full max-w-lg">
        <CardHeader>
          ...
          <CardDescription>
            Sign in with a client key or a username and password. The initial admin key is printed
            once on first boot.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          {loginMode === "key" ? (
            <div className="space-y-2">
              <label className="text-xs font-semibold uppercase tracking-wide">Client key</label>
              <Input
                type="password"
                placeholder="em_..."
                value={inputKey}
          ...
          {error && status !== "error" && <div className="text-sm text-destructive">{error}</div>}
