...
import type { PipelineStepDraft } from "@/features/pipelines/PipelineGraphEditor";
...
export function PipelineComposer() {
  ...
  const { data: pipelineConfig, refetch: refetchPipeline } = useQuery<IngestionPipelineConfig>({
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      const trigger_conditions = artifactType === "any" ? {} : { type: artifactType };
      ...
      <Card className="p-3 space-y-3">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between text-[11px] text-muted-foreground">
            ...
            <Switch checked={showSystemDefaults} onCheckedChange={setShowSystemDefaults} />
          </div>
          {visibleAutomations.length === 0 ? (
            <div className="text-xs text-muted-foreground">No automations configured yet.</div>
          ) : (
      ...
      <Card className="p-4 space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          ...
          <Button
            size="sm"
            variant="ghost"
            disabled={!pipelineConfig?.pipeline_id || setPipelineMutation.isPending}
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <div className="space-y-2">
            ...
            <div className="text-[11px] text-muted-foreground">
              Use “Ingest pipeline (manual)” for the pipeline selected in Settings.
            </div>
          </div>
          <div className="space-y-2">
            ...
            {steps.length === 0 ? (
              ...
              <div className="space-y-3">
                {steps.map((step, index) => (
                  <Card key={`${step.capId}-${index}`} className="p-3 space-y-2">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex-1">
                        <Select
                          value={step.capId}
                          onValueChange={(value) => updateStep(index, { capId: value })}
                        >
                          <SelectTrigger>
                        ...
                        <div className="text-[10px] text-muted-foreground mt-1">
                          {actionCaps.find((cap) => cap.id === step.capId)?.plugin || "plugin"}
                        </div>
                      </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <Textarea
                        ...
                        onChange={(e) => updateStep(index, { inputsText: e.target.value })}
                        className="min-h-24 font-mono text-[11px]"
                      />
                      {inputErrors[index] && (
                        <div className="text-xs text-destructive">{inputErrors[index]}</div>
                      )}
                    </div>
                  </Card>
                ))}
