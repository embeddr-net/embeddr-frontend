...
type PipelineOutgoingEdge = number | { to: number; inputKey?: string; outputKey?: string };

export type PipelineStepDraft = {
  capId: string;
  inputsText: string;
  ui?: { x?: number; y?: number; outgoing?: Array<PipelineOutgoingEdge> };
};

type PipelineGraphEditorProps = {
  steps: Array<PipelineStepDraft>;
  actionCaps: Array<LotusCapability>;
...
export function PipelineGraphEditor({
  ...
  const [manualPositions, setManualPositions] = useState<Record<number, NodePosition>>({});
      ...
      try {
        ...
        if (workflowId || workflowName) {
          ...
          const found = flows.find(
            (w: any) => String(w.id) === String(workflowId) || w.name === workflowName,
          );
      ...
      try {
        ...
        if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
          ...
          if (payload && typeof payload === "object" && !Array.isArray(payload)) {
            return [...baseKeys, ...Object.keys(payload).map((key) => `payload.${key}`)];
          }
          return baseKeys;
        }
      } catch {
      ...
      try {
        ...
        if (workflowId || workflowName) {
          ...
          if (found && found.interface) {
            ...
            // Deduplicate
            if (!exposedLabels.includes("artifact")) exposedLabels.push("artifact");
  ...
  const getNodeKindMeta = (cap?: LotusCapability) => {
    ...
    if (slot.startsWith("event.") || slot.startsWith("workflow.") || slot.startsWith("artifact.")) {
  ...
  const bounds = useMemo(() => {
    ...
    if ((!steps.length && pipelineInputs === undefined) || !Number.isFinite(minX)) {
      return null;
    }
  ...
  const edges = useMemo(() => {
    ...
    steps.forEach((step, index) => {
      ...
      outgoing.forEach((entry) => {
        ...
        if (normalized.to !== index && normalized.to >= 0 && normalized.to < steps.length) {
          derived.push({
            from: index,
            to: normalized.to,
    ...
    if (pipelineInputs) {
      steps.forEach((step, index) => {
        ...
        try {
          const parsed = JSON.parse(step.inputsText || "{}");
          const scanObj = (obj: any, path: Array<string>) => {
  ...
  const handleDisconnect = (index: number, portKey: string, type: "input" | "output") => {
  ...
  const handleConnectionPointerUpCapture = (event: React.PointerEvent<HTMLDivElement>) => {
  ...
  const completeConnection = (toIndex: number, inputKey?: string) => {
    ...
    steps.forEach((step, idx) => {
      ...
      const nextOutgoing = outgoing
        ...
        .filter(
          (entry) => entry.to !== toIndex || (inputKey ? entry.inputKey !== inputKey : false),
        );
    ...
    <div className="space-y-2 h-[95.5%] outline-none" tabIndex={0}>
      <div className="flex items-center justify-between">
        <div className="text-xs text-muted-foreground">
          Drag nodes to rearrange. Hold Shift or use Pan mode to drag the canvas. Ctrl/Alt/⌘ +
          scroll to zoom.
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            disabled={Object.keys(manualPositions).length === 0 && !steps.some((step) => step.ui)}
          >
            Auto layout
          </Button>
        ...
        >
          <svg className="absolute inset-0 h-full w-full z-0 overflow-visible pointer-events-auto">
            ...
            {edges.map((edge) => {
              ...
              const outputIndex = edge.outputKey ? actualOutputKeys.indexOf(edge.outputKey) : 0;
              const inputIndex = edge.inputKey ? inputKeys.indexOf(edge.inputKey) : 0;
            ...
            {connecting &&
              ...
              (() => {
                ...
                const startX = positions[connecting.fromIndex].x + fromDim.width / 2;
                ...
                if (connecting.fromIndex === -1 && pipelineInputs) {
                  ...
                  const keys = outputKeysByIndex[connecting.fromIndex] || ["artifact"];
                  const idx = keys.indexOf(connecting.outputKey || "");
                  if (idx >= 0) startY = getPortY(connecting.fromIndex, idx);
              ...
              <div key={`ports-${index}`} className="relative z-30">
                {inputKeys.length === 0 ? (
                  <div
                    className={`absolute h-3 w-3 rounded-full shadow ${
                      hoveredPort?.nodeIndex === index && hoveredPort?.kind === "input"
                    ...
                    onPointerEnter={() => setHoveredPort({ nodeIndex: index, kind: "input" })}
                    onPointerLeave={() => setHoveredPort(null)}
                    role="button"
                  />
                ) : (
                ...
                {outputKeys.map((key, slotIndex) => (
                  <div
                    ...
                    onPointerEnter={() => setHoveredPort({ nodeIndex: index, kind: "output", key })}
                    onPointerLeave={() => setHoveredPort(null)}
                    role="button"
                  />
                ))}
          ...
          {steps.map((step, index) => {
            ...
            return (
              <PipelineGraphNode
                ...
                onCapChange={(idx, value) => onStepChange(idx, { capId: value })}
              />
            );
          })}
        </div>
          ...
          {editIndex !== null && (
            <div className="h-full space-y-2">
              ...
              <Textarea
                ...
                onChange={(event) => onStepChange(editIndex, { inputsText: event.target.value })}
