...
export function LotusStoragePanel({
  ...
  const resourceAdapters = useMemo(() => {
    const data = resourceAdaptersQuery.data;
  ...
  const [resourceResolveResult, setResourceResolveResult] = useState<Record<string, any> | null>(
    null,
  );
  ...
  const uploadTestMutation = useMutation({
    mutationFn: async () => {
      ...
      try {
        ...
        const complete = await api.lotus.invoke("embeddr-core.artifact.upload.complete", {
          upload_id: uploadId,
          confirm: true,
        });
      ...
      <div className="flex flex-col gap-4">
            ...
            {storageConfigCaps.length === 0 ? (
              ...
              <div className="space-y-2">
                {storageConfigCaps.map((cap) => (
                  <div key={cap.id} className="rounded border border-muted/60 bg-muted/20 p-2">
                    ...
                    <div className="text-[11px] text-muted-foreground">{cap.id}</div>
                  </div>
                ))}
              </div>
            )}
        ...
        <Card className="border-muted/60 bg-transparent">
          ...
          <CardContent className="flex flex-col gap-3 text-xs">
            ...
            <div className="flex flex-col gap-2">
              <span className="text-[11px] text-muted-foreground">Upload provider (optional)</span>
              <Select
                value={uploadProvider || uploadProviderAutoValue}
                onValueChange={(value) =>
                  setUploadProvider(value === uploadProviderAutoValue ? "" : value)
                }
              >
                <SelectTrigger>
            ...
            <div className="flex items-center gap-2">
              <Switch checked={uploadConfirm} onCheckedChange={setUploadConfirm} />
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                disabled={!uploadFile || !uploadConfirm || uploadTestMutation.isPending}
              >
          ...
          <CardContent className="flex flex-col gap-3 text-xs">
            <div className="grid gap-2 md:grid-cols-2">
              <div className="rounded border border-muted/60 bg-muted/20 p-2">
                <div className="text-[10px] text-muted-foreground">Default upload provider</div>
                <div className="text-[11px] font-medium">{defaultBlobProvider || "unset"}</div>
              ...
              <div className="rounded border border-muted/60 bg-muted/20 p-2">
                <div className="text-[10px] text-muted-foreground">Default resolver</div>
                <div className="text-[11px] font-medium">{defaultBlobResolver || "unset"}</div>
              </div>
            </div>
            <div className="grid gap-2 md:grid-cols-2">
              <div className="rounded border border-muted/60 bg-muted/20 p-2">
            ...
            <div className="rounded border border-muted/60 bg-muted/20 p-2">
              <div className="text-[10px] text-muted-foreground">Provider → Resolver mapping</div>
        ...
        <Card className="border-muted/60 bg-transparent">
          ...
          <CardContent className="text-xs">
            {resourceAdapters.length === 0 ? (
              <div className="text-muted-foreground">No resource adapters registered.</div>
            ) : (
              <div className="space-y-2">
                {resourceAdapters.map((adapter) => (
                  <div key={adapter.id} className="rounded border border-muted/60 bg-muted/20 p-2">
                    <div className="flex items-center justify-between">
                      <span className="font-medium">{adapter.title || adapter.id}</span>
                      {adapter.plugin && (
                        <Badge variant="outline" className="text-[10px]">
                          {adapter.plugin}
                        </Badge>
                    ...
                    <div className="text-[11px] text-muted-foreground">{adapter.id}</div>
        ...
        <Card className="border-muted/60 bg-transparent">
          ...
          <CardContent className="flex flex-col gap-3 text-xs">
            <div className="flex flex-col gap-2">
              <span className="text-[11px] text-muted-foreground">Resource URL</span>
              <Input
                value={resourceUrl}
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div className="flex flex-col gap-2">
                <span className="text-[11px] text-muted-foreground">Adapter (optional)</span>
                <Select
                  value={resourceAdapterId || resourceAdapterAutoValue}
                  onValueChange={(value) =>
                    setResourceAdapterId(value === resourceAdapterAutoValue ? "" : value)
                  }
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    <SelectItem value={resourceAdapterAutoValue}>Auto</SelectItem>
                    {resourceAdapters.map((adapter) => (
              ...
              <div className="flex flex-col gap-2">
                <span className="text-[11px] text-muted-foreground">Hint type (optional)</span>
                <Input
            ...
            <div className="flex items-center gap-2">
              ...
              <Button variant="ghost" onClick={() => setResourceResolveResult(null)}>
                Clear
              </Button>
            </div>
