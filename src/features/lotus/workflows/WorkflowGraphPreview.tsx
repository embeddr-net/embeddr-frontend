...
function extractBindings(value: unknown): Array<string> {
    ...
    Object.entries(outputBindings || {}).forEach(([outputKey, binding]) => {
      extractBindings(binding).forEach((token) => {
        if (token.startsWith("inputs.") || token.startsWith("steps.")) {
          const source = token.startsWith("inputs.") ? token : `step.${token.split(".")[1]}`;
          edges.push({
            from: source,
      ...
      <div className="grid gap-2 md:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1">
            {links.length === 0 && <Badge variant="secondary">No links yet</Badge>}
            {links.map((edge, index) => (
              <div key={`${edge.from}-${edge.to}-${index}`} className="text-[10px]">
