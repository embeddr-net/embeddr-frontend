...
import type { CapabilityPort } from "./CapabilityDialog";
...
export function WorkflowEditorContent({
  ...
  const actionOutputOptions = useMemo(() => {
    const map = new Map<string, Array<CapabilityPort>>();
    capsById.forEach((cap) => {
      const schema = cap?.data?.output?.schema as { properties?: Record<string, any> } | undefined;
  ...
  return (
    <ScrollArea className="h-full pr-2">
      {!selectedWorkflow && <div className="text-muted-foreground">Select a workflow to edit.</div>}
        ...
        <>
          {!draftMeta.workflow && (
            <div className="rounded-md border border-muted/60 bg-muted/30 p-3 text-[11px] text-muted-foreground">
              This workflow is provided by a plugin interface. Input/output editing is read-only
              until it is migrated to the workflow schema.
            </div>
          )}
            ...
            <div className="rounded-md border border-muted/60 bg-card/40 p-3 space-y-3">
              ...
              {draftMeta.interface?.exposed_inputs?.length ? (
                <div className="space-y-2">
                  <div className="text-[11px] text-muted-foreground">Exposed Inputs</div>
                  {draftMeta.interface.exposed_inputs.map((inp: any, idx: number) => (
                    <div
                    ...
                    >
                      ...
                      {inp.description && (
                        <div className="text-[10px] text-muted-foreground">{inp.description}</div>
                      )}
                    </div>
                  ))}
                </div>
            ...
            <div className="space-y-3">
                ...
                <div className="space-y-1">
                  {primitiveLibrary.map((primitive) => {
                    ...
                    return (
                      ...
                      >
                        <span className="text-xs font-medium">{primitive.title}</span>
                        {!isLoaded && <Badge variant="secondary">not loaded</Badge>}
                      </Button>
                    );
                  })}
                </div>
              ...
              <ScrollArea className="h-80">
                <div className="space-y-2 pr-2">
                  {actionCaps.map((cap) => (
                    <div key={cap.id} className="rounded-md border border-muted/60 p-2">
                      <div className="text-xs font-medium">{cap.title || cap.id}</div>
                      ...
                      <Button
                        ...
                        onClick={() =>
                          openCapabilityDialog({
                            ...
                            inputs: getActionInputOptions(cap.id).map((opt) => ({
                              name: opt.key,
                              type: opt.type,
                  ...
                  {actionCaps.length === 0 && (
                    <div className="text-xs text-muted-foreground">No Lotus actions found.</div>
                  )}
                </div>
              </ScrollArea>
            </div>
            ...
            <div className="space-y-3">
              <div className="rounded-md border border-muted/60 p-3 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Button size="sm" variant="outline" onClick={() => onAddPort("inputs")}>
                      Add input
                    </Button>
                    <Button size="sm" variant="outline" onClick={() => onAddPort("outputs")}>
                      Add output
                    </Button>
                  </div>
                </div>
                <div className="space-y-3">
                  <div className="space-y-2">
                    ...
                    {Object.entries(draftMeta.workflow?.inputs || {}).map(([key, port]) => (
                      <div key={key} className="rounded-md border border-muted/60 p-2 space-y-2">
                        <div className="flex items-center gap-2">
                          <Input
                    ...
                    {Object.keys(draftMeta.workflow?.inputs || {}).length === 0 && (
                  ...
                  <div className="space-y-2">
                    ...
                    {Object.entries(draftMeta.workflow?.outputs || {}).map(([key, port]) => (
                      <div key={key} className="rounded-md border border-muted/60 p-2 space-y-2">
                        <div className="flex items-center gap-2">
                          <Input
                            value={(port as any).name || key}
                        ...
                        <Input
                          value={draftMeta.workflow?.output_bindings?.[key] || ""}
                          onChange={(event) => onUpdateOutputBinding(key, event.target.value)}
                          placeholder="binding (e.g. ${'{'}steps.0.image_id${'}'})"
                          className="text-xs"
                        />
                      </div>
                    ))}
                    {Object.keys(draftMeta.workflow?.outputs || {}).length === 0 && (
                      <div className="text-xs text-muted-foreground">
              ...
              <div className="flex items-center justify-between">
                ...
                <Badge variant="outline">
                  {draftMeta.workflow?.implementation?.payload?.steps?.length || 0}
                </Badge>
              </div>
              {!draftMeta.workflow && (
                <Button variant="outline" onClick={() => setDraftMeta(ensureFlowDraft(draftMeta))}>
                  Convert to Flow
                </Button>
              )}
              {draftMeta.workflow && (
                <div className="space-y-2">
                  <WorkflowGraphPreview
                    steps={draftMeta.workflow?.implementation?.payload?.steps || []}
                    capsById={capsById}
                  ...
                  />
                  {(draftMeta.workflow.implementation?.payload?.steps || []).map(
                    (step: any, index: number) => {
                      const cap = capsById.get(step.capability_id);
                      const capPlugin = (cap as any)?.plugin;
                      const actionInputs = getActionInputOptions(step.capability_id);
                      const workflowInputs = Object.keys(draftMeta.workflow?.inputs || {});
                      const previousOutputs = Array.from({ length: index }, (_, i) =>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between gap-2">
                            <div>
                              ...
                              <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
                                {capPlugin && <Badge variant="outline">{capPlugin}</Badge>}
                                <span className="font-mono">{step.capability_id}</span>
                              </div>
                            </div>
                          ...
                          <div className="space-y-2">
                              ...
                              <div key={key} className="grid gap-2 md:grid-cols-2">
                                <Select
                                  ...
                                  onValueChange={(nextKey) => {
                                    ...
                                    const inputs =
                                      next.workflow.implementation.payload.steps[index].inputs ||
                                      {};
                                    delete inputs[key];
                                ...
                                <div className="flex gap-2">
                                  <Input
                                    value={String(value ?? "")}
                                    onChange={(event) =>
                                      onStepInputChange(index, key, event.target.value)
                                    }
                                    className="text-xs"
                                  />
                                  <Select
                                    onValueChange={(picked) =>
                                      onStepInputChange(index, key, `\${'{'}${picked}\${'}'}`)
                                    }
                                  >
                                    ...
                                    <SelectContent>
                                      {workflowInputs.map((port) => (
                                        <SelectItem key={`inputs.${port}`} value={`inputs.${port}`}>
                                          inputs.{port}
                                        </SelectItem>
                                      ))}
                                  ...
                                  <Button
                                    size="icon"
                                    variant="ghost"
                                    onClick={() => onRemoveStepInput(index, key)}
                                  >
                                    <X className="h-3 w-3" />
                                  </Button>
                                  <Button
                            ...
                            <div className="text-[10px] text-muted-foreground">
                              Use ${"{inputs.name}"} or ${"{steps.0.output}"} in values.
                            </div>
                            <div className="flex flex-wrap items-center gap-2">
                              <Button size="sm" variant="outline" onClick={() => onTestStep(index)}>
                                Test Step
                              </Button>
                              {stepOutputs[index] && (
