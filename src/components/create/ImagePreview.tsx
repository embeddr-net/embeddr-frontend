...
export function ImagePreview({ children }: { children?: React.ReactNode }) {
  ...
  const [activeVersion, setActiveVersion] = useState<"output" | "parent">("output");
  const [customComparisonImage, setCustomComparisonImage] = useState<string | null>(null);
  const [customPrimaryImage, setCustomPrimaryImage] = useState<string | null>(null);
  const [isComparisonSelectorOpen, setIsComparisonSelectorOpen] = useState(false);
  const [selectionTarget, setSelectionTarget] = useState<"primary" | "secondary">("secondary");

  // Extract parent image
  const inputValues = Object.values(selectedGeneration?.inputs || {});
  const parentInput = inputValues.find((val: any) => val?.image_id || val?._preview);
  ...
  const imageToShow =
    customPrimaryImage ||
    (selectedImage
      ? selectedImage.image_url || `${BACKEND_URL}/images/${selectedImage.id}/file`
      : null) ||
    (activeVersion === "parent" && hasParent ? parentPreview : currentImage || previewImage);
        ...
        <div className="flex-1 m-0 overflow-hidden bg-card flex items-center justify-center bg-muted/20 relative group/image-preview">
          {selectedGeneration ? (
            selectedGeneration.status === "completed" && currentImage ? (
                      ...
                      >
                        {imageToShow ? (
                          /\.(mp4|webm|mov|mkv)$/i.test(imageToShow) ? (
                            ...
                            <img src={imageToShow} className="w-full h-full object-cover" alt="R" />
                          )
                        ) : (
                          "R"
                        )}
              ...
              <div className="text-center space-y-4">
                ...
                <p className="text-muted-foreground text-sm">
                  {selectedGeneration.status === "pending" ? "Pending..." : "Generating..."}
                </p>
            ...
            <div className="text-center space-y-4">
              ...
              <p className="text-muted-foreground text-sm">Generated images will appear here</p>
              {children}
            </div>
          )}
        </div>
