...
export function ParentArtifactSelector({
  value,
  onChange,
}: ParentArtifactSelectorProps) {
  ...
  const displayName = selectedArtifact
    ? selectedArtifact.metadata_json?.name ||
      selectedArtifact.metadata_json?.filename ||
      'Artifact'
    : value
      ? 'Selected (UUID)'
      : 'Select parent...'
          ...
          <div className="max-h-[200px] overflow-y-auto flex flex-col gap-1">
            ...
            {results.map((art) => (
              <Button
                ...
                size="sm"
                className={cn(
                  'justify-start h-8 font-normal',
                  value === art.id && 'bg-accent',
                )}
                onClick={() => {
