...
export function GlobalEffectsLayer() {
  ...
  if (!effectsEnabled || !globalEffectsEnabled || effects.length === 0) return null;

  return (
    <div className="zen-effects-layer pointer-events-none fixed inset-0 z-[70]">
      {effects.map(({ pluginId, def }) => {
        ...
        if (def?.props?.scope !== "global") return null;
        return (
          <div
