...
export function TransportAccessSettings() {
  ...
  const { data: policyResponse, isLoading: isPolicyLoading } = useQuery({
    ...
    queryFn: () =>
      embeddrApi.config.get('embeddr-core', { config_id: POLICY_CONFIG_ID }),
  })
  ...
  const transports = useMemo<TransportDescriptor[]>(() => {
    const base: TransportDescriptor[] = [
  ...
  const enabledTransports = useMemo(() => {
    ...
    return transports.filter(
      (transport) => !transport.plugin || set.has(transport.plugin),
    )
  }, [plugins, transports])
        ...
        <div className="flex items-center justify-between">
          ...
          <Switch
            ...
            onCheckedChange={(value) =>
              setPolicy((prev) => ({ ...prev, enabled: Boolean(value) }))
            }
          />
        </div>
