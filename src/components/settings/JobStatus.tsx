...
import { Button, Progress } from "@embeddr/react-ui/ui";
  ...
  return (
    <div
      className={`border p-4 space-y-3 transition-colors ${
        isCompleted ? " border-green-200" : isFailed ? "bg-red-50/50 border-red-200" : "bg-muted/50"
      }`}
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground ml-2">{status.message}</span>
        </div>
