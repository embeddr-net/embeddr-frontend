...
import {
  fetchPetals,
  installPetalPluginDeps,
  type InstallPluginDepsResult,
  type PetalInfo,
  type PetalPluginHealth,
...
function PluginHealthRow({
  ...
  const [lastResult, setLastResult] = useState<InstallPluginDepsResult | null>(
    null,
  )
  ...
  return (
    ...
    >
      ...
      {item.error && (
        <div className="mt-1 text-destructive/80">{item.error}</div>
      )}
      {item.missing_deps && item.missing_deps.length > 0 && (
        <div className="mt-1 text-muted-foreground">
          Missing:{' '}
          <span className="font-mono">{item.missing_deps.join(', ')}</span>
        </div>
      )}
      ...
      {lastResult && showOutput && (
        <div className="mt-2 space-y-1.5">
          <div
            className={`text-[11px] ${
              lastResult.ok ? 'text-green-500' : 'text-destructive'
            }`}
          >
            {lastResult.ok
              ? 'Install succeeded'
...
export function PetalsPanel() {
  ...
  const degradedCount =
    petals?.reduce(
      (sum, p) =>
        sum +
        (p.plugin_health || []).filter((h) => isDegraded(h.status)).length,
      0,
    ) ?? 0
