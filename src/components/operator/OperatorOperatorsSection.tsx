...
import type { fetchSecurityOperators } from "@/lib/api";

interface OperatorOperatorsSectionProps {
  operators: Array<SecurityOperator>;
  operatorsQuery: UseQueryResult<Awaited<ReturnType<typeof fetchSecurityOperators>>>;
  isForbidden: boolean;
}
    ...
    {isForbidden ? (
      <Card className="border-destructive/40">
        <CardHeader>
          ...
          <CardDescription>You do not have permission to view operators.</CardDescription>
        </CardHeader>
      </Card>
    ) : (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Root operators are reserved for server administration.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          {operatorsQuery.isLoading ? (
            <div className="text-sm text-muted-foreground">Loading operators…</div>
          ) : operatorsQuery.isError ? (
            <div className="text-sm text-destructive">Unable to load operators.</div>
          ) : operators.length === 0 ? (
            <div className="space-y-2 text-sm text-muted-foreground">
              <div>No operators found.</div>
              <div>
                If this is an older workspace, run the bootstrap flow or migration to create root
                and user operators.
              </div>
            </div>
          ) : (
              ...
              <div key={op.id} className="rounded border p-3">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-medium">{op.display_name || op.name}</div>
                ...
                <div className="mt-3 grid gap-2 text-xs text-muted-foreground sm:grid-cols-2">
                  <div>
                    Clients: {op.user_count} total, {op.active_user_count} active
                  </div>
                  ...
                  {op.created_at ? (
                    <div>Created {new Date(op.created_at).toLocaleDateString()}</div>
                  ) : null}
                </div>
              </div>
            ))
