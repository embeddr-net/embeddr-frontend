...
export function ThemeSynchronizer() {
  const { themeMode, themePackLightId, themePackDarkId, customCss, customCssEnabled } =
    useSettingsStore();
  ...
  const activePackId = resolveMode === "dark" ? themePackDarkId : themePackLightId;
  const activePack =
    activePackId && activePackId !== "default"
      ? packs.find((pack) => pack.id === activePackId) || null
      : null;

  const getTokensForMode = (pack: typeof activePack, modeKey: "light" | "dark") => {
    if (!pack?.tokens) return null;
    return pack.tokens[modeKey] || pack.tokens[modeKey === "dark" ? "light" : "dark"] || null;
  };
  ...
  useEffect(() => {
    ...
    const handleChange = () => {
      ...
      const nextPack =
        packId && packId !== "default" ? packs.find((pack) => pack.id === packId) || null : null;
