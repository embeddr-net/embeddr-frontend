...
export const PluginsList = () => {
  ...
  const [selectedAction, setSelectedAction] = useState<ActionDialogState | null>(null);
  ...
  const actions = useMemo(() => {
    const items = (data?.items || []) as Array<LotusActionCapability>;
  ...
  const grouped = useMemo(() => {
    const map = new Map<string, Array<LotusActionCapability>>();
    actions.forEach((cap) => {
    ...
    return grouped
      .map(([plugin, caps]) => {
        const filteredCaps = caps.filter((cap) => {
          const hay = `${cap.id} ${cap.title} ${cap.description || ""}`.toLowerCase();
  ...
  if (error) {
    return <div className="p-4 text-red-500">Error loading Lotus actions: {error.message}</div>;
  }
                ...
                {caps.map((cap) => (
                  ...
                  >
                    <div className="flex flex-col">
                      <span className="text-sm font-medium">{cap.title || cap.id}</span>
                      {cap.description && (
      ...
      <Dialog open={!!selectedAction} onOpenChange={(open) => !open && setSelectedAction(null)}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedAction?.capability.description || selectedAction?.capability.id}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            {selectedAction &&
              (() => {
                const schema = selectedAction.capability.data?.input?.schema || {};
                const properties = schema.properties || {};
                    ...
                    return (
                      <div key={key} className="flex items-center justify-between">
                        ...
                        <Switch
                          ...
                          onCheckedChange={(checked) => updateInput(key, prop, checked)}
                        />
                      </div>
                    );
                  }
                    ...
                    return (
                      <div key={key} className="space-y-2">
                        ...
                        <Textarea
                          ...
                          onChange={(event) => updateInput(key, prop, event.target.value)}
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) =>
                    setSelectedAction(
                      selectedAction ? { ...selectedAction, confirm: checked } : selectedAction,
                    )
                  }
                />
              </div>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => selectedAction && invokeMutation.mutate(selectedAction)}
              disabled={invokeMutation.isPending || !selectedAction}
            >
              {invokeMutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              Invoke
            </Button>
          </DialogFooter>
        </DialogContent>
