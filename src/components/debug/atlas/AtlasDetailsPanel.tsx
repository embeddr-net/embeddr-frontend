...
import { Badge, Input, ScrollArea, Separator, Spinner, VideoPlayer } from "@embeddr/react-ui";
import {
  ArrowLeft,
  ArrowRight,
  Code,
...
import type { GraphEdge, Point3D } from "./types";
    ...
    return (
      <div className="w-80 h-full border-l bg-card flex flex-col shadow-xl z-20 shrink-0 overflow-hidden">
        <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground p-4 text-center">
          ...
          <p className="text-sm">Select a node in the 3D view to inspect details and relations.</p>
        </div>
      </div>
    );
  }
    ...
    if (details.type_name === "image") {
      return (
        <div className="aspect-square bg-muted  overflow-hidden border">
          <img src={safeUrl(details.id)} className="w-full h-full object-contain" />
        </div>
      );
    }
    if (
    ...
    if (
      ...
      return (
        <div className="aspect-square bg-muted overflow-hidden border flex items-center justify-center p-3">
          <audio src={safeUrl(details.id)} controls preload="metadata" className="w-full" />
        </div>
      );
    }
    ...
    if (details.type_name === "text" || details.base_type_name === "text") {
      return (
        <div className="h-48 bg-muted  p-2 border overflow-hidden text-xs font-mono">
          <iframe src={safeUrl(details.id)} className="w-full h-full bg-transparent" />
        </div>
      );
    }
    return (
      ...
      <div className="flex flex-col h-full">
        <div className="p-4 border-b">
          <div className="flex items-center gap-2 mb-2">
            <Badge variant="outline">{details?.type_name || "Loading..."}</Badge>
          ...
          <div className="p-4 space-y-6 pb-20">
            ...
            details?.metadata_json?.uri === "virtual://search" ? (
              <div className="space-y-4">
                ...
                <ScrollArea className="h-[400px] border pr-2" type="always" variant="left-border">
                  <div className="flex flex-col gap-1 p-1">
                    {details.metadata_json.results &&
                      (details.metadata_json.results as Array<string>).map((id) => {
                        const point = points.find((p) => p.id === id);
                        const name = point?.label || id.toString().substring(0, 8);
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-2 overflow-hidden">
                              ...
                              />
                              <span className="truncate whitespace-pre-wrap break-all">{name}</span>
                            </div>
              ...
              <>
                      ...
                      <div className="text-lg font-bold">
                        {
                          new Set(
                            graphEdges.map(
                              (e) =>
                                [e.source, e.target].sort().join("-") + e.type + (e.label || ""),
                            ),
                          ).size
                        }
                      </div>
                      <div className="text-[9px] text-muted-foreground uppercase">Relations</div>
                    </div>
                    <div className="bg-muted/50 p-2  text-center">
                      <div className="text-lg font-bold">
                        {new Set(graphEdges.flatMap((e) => [e.source, e.target])).size}
                      </div>
                      <div className="text-[9px] text-muted-foreground uppercase">Nodes</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {details?.override_capabilities?.map((c: string) => (
                      <Badge key={c} variant="secondary" className="text-[10px]">
                        {c}
                      </Badge>
                    ))}
                    {!details && <div className="text-xs text-muted-foreground">Loading...</div>}
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-muted-foreground">
                      {
                        graphEdges.filter(
                          (e) => e.source === selectedPoint.id || e.target === selectedPoint.id,
                        ).length
                      }
                    </span>
                  </div>
                  ...
                  <ScrollArea className="h-48 border  pr-2" type="always" variant="left-border">
                    <div className="p-1 flex flex-col gap-1">
                      {(() => {
                        ...
                        return graphEdges
                          ...
                          .map((edge, idx) => {
                            const otherId =
                              edge.source === selectedPoint.id ? edge.target : edge.source;
                            ...
                            if (inGraph) {
                              ...
                              const artifact = artifacts?.find((a) => a.id === otherId);
                              if (artifact) {
                            ...
                            if (
                              connectionSearch &&
                              !targetName.toLowerCase().includes(connectionSearch.toLowerCase()) &&
                              !typeLabel.toLowerCase().includes(connectionSearch.toLowerCase()) &&
                              !targetType.toLowerCase().includes(connectionSearch.toLowerCase())
                            ) {
                              return null;
                            }
