...
import type { GraphEdge, Point3D, SearchResult } from "./atlas/types";
import { cn } from "@/lib/utils";
import { embeddrApi } from "@/lib/api/client";

// Simple cache for projection results
const PROJECTION_CACHE = new Map<string, Array<Point3D>>();
...
export const VectorAtlas = () => {
  ...
  const [activeSearches, setActiveSearches] = useState<Array<SearchResult>>([]);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const color = palette.find((c) => !usedColors.has(c)) || vizColors.searchPalette[0];
      ...
      const semanticRes = await embeddrApi.plugins
        ...
        .catch(() => ({ items: [] as Array<{ id: string; score: number }> }));
  ...
  useEffect(() => {
    ...
    const traverse = async () => {
      ...
      if (selectedPoint.id.toString().startsWith("search-result-")) {
        ...
        if (search) {
          const edges: Array<GraphEdge> = search.items.map((itemId) => ({
      ...
      try {
        ...
        const subgraph = (await embeddrApi.artifacts.getSubgraph(selectedPoint.id as string, {
          maxDepth: depth[0],
          includeLineage: true,
        ...
        if (isMounted) {
          ...
          const edges: Array<GraphEdge> = subgraph.edges.map((e: any) => ({
            source: e.source,
            target: e.target,
            type: e.type,
            label: e.label,
  ...
  const { data: debugRelations } = useQuery({
    ...
    queryFn: () =>
      selectedPoint ? embeddrApi.artifacts.getRelations(selectedPoint.id as string) : [],
    enabled: !!selectedPoint && !selectedPoint.id.toString().startsWith("search-result-"),
  });

  // Perform UMAP Projection
  const rawPoints: Array<Point3D> = useMemo(() => {
    if (useBackend) {
      if (backendPoints) return backendPoints as Array<Point3D>;
      return [];
    }
    ...
    try {
      ...
      const featureMatrix = artifacts.map((a: any) => {
        ...
        const source = a.metadata_json?.source || "";
      ...
      const projected = artifacts.map((a: any, i: number) => {
        return {
          ...
          label:
            ...
            a.metadata_json?.title ||
            a.metadata_json?.original_filename ||
  ...
  }, [artifacts, nNeighbors, minDist, spread, useBackend, backendPoints, vizColors]);
  ...
  const { points, searchMarkers } = useMemo(() => {
      ...
      res = res.filter((p: any) => {
        ...
        if (Array.isArray(cols)) {
          return cols.some((c: any) => {
            ...
            if (typeof c === "object" && c?.id) return c.id === selectedCollection;
            return false;
          });
        }
        return cols === selectedCollection || cols?.id === selectedCollection;
      });
    }
    ...
    if (activeSearches.length > 0) {
      ...
      const generatedSearchPoints: Array<Point3D> = [];
      ...
      activeSearches.forEach((search) => {
        ...
        const hasBackendPoint = res.some((p) => p.label === `Search: ${search.query}`);

        if (!hasBackendPoint) {
          // Only use fallback centroid if backend didn't provide one
          const matchingPoints = res.filter((p) => search.items.includes(p.id.toString()));
      ...
      <div className="flex-1 relative h-full min-w-0">
        ...
        <div className="absolute top-2 right-2 z-10 bg-card/80 border border-border p-2 backdrop-blur text-[10px] text-foreground space-y-1 pointer-events-none">
          <div className="flex items-center gap-1">
            <div className="w-2 h-2" style={{ backgroundColor: vizColors.connectionParent }}></div>{" "}
            Parent (Input/Source)
          </div>
          <div className="flex items-center gap-1">
            <div className="w-2 h-2" style={{ backgroundColor: vizColors.connectionChild }}></div>{" "}
        ...
        <Umap3DExplorer
          ...
          getImageUrl={(point, type) => {
            if (type === "thumb") {
              return embeddrApi.artifacts.getPreviewUrl((point as any).id, "thumbnail");
            }
