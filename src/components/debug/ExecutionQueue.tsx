...
import { useCreateExecution, useExecutionEvents, useExecutions } from "@/hooks/useExecutions";
...
const ExecutionQueue = () => {
  ...
  const { data: executions = [], refetch, isLoading } = useExecutions(execFilters);
  const queryClient = useQueryClient();
  const { mutateAsync: createExecution } = useCreateExecution();
  const [rootEvents, setRootEvents] = useState<Record<string, Array<ExecutionEvent>>>({});
  const [expandedRows, setExpandedRows] = useState<Record<string, boolean>>({});
  const [rowEvents, setRowEvents] = useState<Record<string, Array<ExecutionEvent>>>({});
  const selectedExecution = useMemo(
    () => executions.find((ex) => ex.id === selectedId) || null,
    [executions, selectedId],
  );
  const { data: events = [], isLoading: eventsLoading } = useExecutionEvents(selectedId, {
    limit: 200,
    offset: 0,
  });
  const executionTree = useMemo(() => buildExecutionTree(executions), [executions]);
  ...
  useEffect(() => {
    ...
    const unsubCompleted = globalEventBus.on("execution.completed", handleMessage);
    ...
    const unsubCreatedCol = globalEventBus.on("execution:created", handleMessage);
    const unsubStartedCol = globalEventBus.on("execution:started", handleMessage);
    const unsubUpdatedCol = globalEventBus.on("execution:updated", handleMessage);
    const unsubCompletedCol = globalEventBus.on("execution:completed", handleMessage);
    const unsubWaitingCol = globalEventBus.on("execution:waiting", handleMessage);
    const unsubResumedCol = globalEventBus.on("execution:resumed", handleMessage);
      ...
      const results = await Promise.all(
        subtreeIds.map(async (id) => {
          try {
            ...
            return [id, [] as Array<ExecutionEvent>] as const;
          }
        }),
      );
      if (cancelled) return;
      const map: Record<string, Array<ExecutionEvent>> = {};
    ...
    <div className="flex h-full min-h-0 flex-col gap-1">
      ...
      <div className="grid flex-1 min-h-0 grid-cols-1 gap-1 lg:grid-cols-[2fr_1fr]">
        <div className="flex-1 min-w-0 min-h-0">
          <ScrollArea className="h-full pr-3 " variant="left-border" type="always">
            <Accordion type="multiple" defaultValue={["running", "pending"]} className="space-y-1">
              {/* Running Section */}
              <AccordionItem value="running" className="border rounded-md px-4 bg-background">
                ...
                <AccordionContent>
                  <ExecutionTable
                    ...
                    emptyMessage={isLoading ? "Loading..." : "No active jobs running."}
              ...
              {/* Pending Section */}
              <AccordionItem value="pending" className="border rounded-md px-4 bg-background">
                <AccordionTrigger className="hover:no-underline py-3">
                  <div className="flex items-center gap-2">
              ...
              <AccordionItem value="failed" className="border rounded-md px-4 bg-background">
                <AccordionTrigger className="hover:no-underline py-3">
                  <div className="flex items-center gap-2">
              ...
              >
                ...
                <AccordionContent>
                  <ExecutionTable
                    ...
                    emptyMessage={isLoading ? "Loading..." : "No completed jobs recorded."}
                    selectedId={selectedId}
                    onSelect={setSelectedId}
                    expandedRows={expandedRows}
                    rowEvents={rowEvents}
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div>
                        ...
                        <div className="font-medium text-sm">{selectedExecution.type}</div>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Plugin</span>
                        <div className="font-medium text-sm">{selectedExecution.plugin_name}</div>
                      </div>
                      <div>
                    ...
                    <Accordion type="multiple" className="space-y-2">
                      <AccordionItem value="inputs" className="border rounded-md">
                      ...
                      <AccordionItem value="outputs" className="border rounded-md">
                        <AccordionTrigger className="px-3 py-2 text-xs hover:no-underline">
                          Outputs
                        </AccordionTrigger>
                        <AccordionContent className="px-3 pb-3">
                      ...
                      <AccordionItem value="events" className="border rounded-md">
                        <AccordionTrigger className="px-3 py-2 text-xs hover:no-underline">
                          Event Trace
                        </AccordionTrigger>
                        <AccordionContent className="px-3 pb-3">
...
const ExecutionTable = ({
  ...
  rowEvents: Record<string, Array<ExecutionEvent>>;
  onToggleRow: (id: string) => void;
}) => {
  if (items.length === 0) {
    return (
      <div className="text-sm text-muted-foreground py-4 text-center italic">{emptyMessage}</div>
    );
  }
        ...
        <TableBody>
          {items.map((ex) => (
            <React.Fragment key={ex.id}>
              <TableRow
                className={cn("cursor-pointer", selectedId === ex.id && "bg-muted/40")}
              ...
              >
                ...
                <TableCell>
                  <div className="flex flex-col">
                    <span className="font-medium text-xs truncate">{ex.type}</span>
                    ...
                    <div className="h-1 w-full bg-secondary rounded-full overflow-hidden">
                      <div
                        className={cn(
                          ...
                          ex.status === "failed" ? "bg-destructive" : "bg-primary",
                        )}
              ...
              {expandedRows[ex.id] ? (
                <TableRow className="bg-muted/20">
                  <TableCell colSpan={6} className="py-2">
                    <div className="text-[11px] text-muted-foreground mb-1">Tool calls</div>
                    <div className="space-y-1">
                      {(rowEvents[ex.id] || [])
                        .filter((event) => event.event_type.startsWith("tool.call"))
                        .map((event) => (
...
const PhoenixTimeline = ({
  ...
  rootId: string | null;
  executionTree: Array<ExecutionNode>;
  eventsByExecution: Record<string, Array<ExecutionEvent>>;
  onSelect: (id: string) => void;
  selectedId: string | null;
}) => {
  ...
  if (!rootNode) {
    return <div className="text-xs text-muted-foreground">Select a job to view its timeline.</div>;
  }

  return (
    <div className="space-y-2">
      <div className="text-xs uppercase tracking-widest text-muted-foreground">Root timeline</div>
    ...
    <div className="space-y-1">
      <button
        className={cn(
          ...
          selectedId === node.execution.id ? "bg-muted/50" : "hover:bg-muted/30",
        )}
      ...
      >
        ...
        <div className="text-[10px] text-muted-foreground">
          {formatDuration(node.execution.started_at, node.execution.finished_at)}
        </div>
      </button>
...
const ToolEventRow = ({ event }: { event: ExecutionEvent }) => {
  ...
  const canReplay = event.event_type === "tool.call.start" && typeof toolName === "string";
...
type ExecutionNode = {
  execution: Execution;
  children: Array<ExecutionNode>;
};

const buildExecutionTree = (items: Array<Execution>): Array<ExecutionNode> => {
  const nodes = new Map<string, ExecutionNode>();
  items.forEach((ex) => {
    nodes.set(ex.id, { execution: ex, children: [] });
  });

  const roots: Array<ExecutionNode> = [];
  nodes.forEach((node) => {
  ...
  const sortNodes = (list: Array<ExecutionNode>) => {
    list.sort(
      (a, b) =>
        new Date(b.execution.created_at).getTime() - new Date(a.execution.created_at).getTime(),
    );
...
const findNode = (nodes: Array<ExecutionNode>, id: string): ExecutionNode | null => {
...
const collectSubtreeIds = (nodes: Array<ExecutionNode>, rootId: string) => {
...
const EventRow = ({ event }: { event: ExecutionEvent }) => {
  const canReplay = event.event_type.startsWith("ui:") || event.event_type.startsWith("ui.");
    ...
    <div className="flex flex-col gap-1 text-[11px] border-b pb-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 min-w-0">
          <span className={cn("font-medium truncate", color)}>{event.event_type}</span>
          {canReplay && (
            <Button
              size="sm"
              variant="ghost"
...
const KeyValueList = ({ value }: { value: any }) => {
  ...
  if (typeof value !== "object") {
    return <div className="wrap-break-word whitespace-pre-wrap">{String(value)}</div>;
  }
  ...
  return (
    <div className="space-y-1 min-w-0 max-w-full">
      {entries.map(([key, val]) => (
        <div key={key} className="grid grid-cols-[120px_minmax(0,1fr)] gap-2 min-w-0">
          <div className="text-[10px] uppercase text-muted-foreground truncate">{key}</div>
...
const matchesFilters = (execution: Execution, filters: any) => {
  ...
  if (filters.plugin_name && !execution.plugin_name.includes(filters.plugin_name)) return false;
