...
export function BottomPanel({ items, activeTab, onTabChange, className }: BottomPanelProps) {
  const activeItem = items.find((item) => item.id === activeTab);
  const [isExpanded, setIsExpanded] = React.useState(false);

  return (
    <Card className={cn("flex flex-col bg-background shrink-0 p-0! gap-0!", className)}>
      {/* Content Area */}
      ...
      <div className="flex items-center gap-1 p-1 bg-muted/40">
        {items.map((item) => (
          <Button
            ...
            size="sm"
            onClick={() => !item.disabled && onTabChange(activeTab === item.id ? null : item.id)}
