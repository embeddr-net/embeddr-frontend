...
export const usePluginWorkflow = (pluginName: string | undefined, workflowName: string | null) => {
  const resolvedPluginName = useWorkflowProviderPluginId();
  const targetPlugin = pluginName || resolvedPluginName;
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(`${BACKEND_URL}/plugins/${targetPlugin}/workflows/${workflowName}`);
...
export const useRunPluginWorkflow = (pluginName?: string) => {
  ...
  return useMutation({
    mutationFn: async ({ workflowName, inputs }: { workflowName: string; inputs: any }) => {
