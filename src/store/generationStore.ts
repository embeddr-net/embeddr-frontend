...
interface GenerationState {
  ...
  generate: (patch?: { nodeId: string; field: string; value: any }) => Promise<void>;
  ...
  persist(
    (set, get) => ({
      ...
      initEventListeners: () => {
            ...
            set((state) => {
              ...
              running_generations.forEach((gen: any) => {
                ...
                if (images.length === 0 && outputs.length > 0) {
                  images = outputs
                    .filter((o: any) => o.type === "image" && o.comfy_type !== "temp")
                    .map(
                      (o: any) =>
        ...
        const handleWorkflowExecutionStart = (data: any) => {
          ...
          set((state) => ({
            ...
            currentPreview: null,
            generations: state.generations.map((g) =>
              g.prompt_id === data.prompt_id ? { ...g, status: "processing" } : g,
            ),
          }));
        ...
        const handleWorkflowExecuted = (data: any) => {
          ...
          if (gen) {
            ...
            const embeddrImages = embeddrIds.map((id: any) => `${BACKEND_URL}/images/${id}/file`);
            ...
            const isLatest = get().generations.length > 0 && get().generations[0].id === gen.id;
        ...
        workflowSourceAliases.forEach((source) => {
          ...
          globalEventBus.on(`${source}:execution_start`, handleWorkflowExecutionStart);
          globalEventBus.on(`${source}:preview`, handleWorkflowPreview);
          globalEventBus.on(`${source}:execution_error`, handleWorkflowExecutionError);
      ...
      selectGeneration: (generation) => {
        console.log("[GenerationStore] selectGeneration called for:", generation.id);
        set({
      ...
      toggleFollowLatest: () => set((state) => ({ followLatest: !state.followLatest })),

      selectNextGeneration: () => {
        const { generations, selectedGenerationId } = get();
        if (!selectedGenerationId || generations.length === 0) return;
        const index = generations.findIndex((g) => g.id === selectedGenerationId);
      ...
      fetchWorkflows: async () => {
        ...
        try {
          ...
          if (data.length > 0) {
            const last = data.find((w: any) => w.id.toString() === lastWorkflowId);
      ...
      fetchHistory: async (skip = 0) => {
        ...
        try {
          const res = await fetch(`${BACKEND_URL}/generations?limit=20&skip=${skip}`);
          if (!res.ok) return;
          const data = await res.json();
          ...
          set((state) => {
            ...
            if (state.followLatest && newGenerations.length > 0) {
              const latestCompleted = newGenerations.find((g) => g.status === "completed");
              if (latestCompleted && state.selectedGenerationId !== latestCompleted.id) {
                return {
                  generations: newGenerations,
      ...
      loadMoreHistory: () => {
        const { hasMoreHistory, isLoadingHistory, historySkip, fetchHistory } = get();
      ...
      generate: async (patch) => {
        ...
        if (selectedWorkflow.data) {
          if (Array.isArray((selectedWorkflow.data as any).nodes)) {
            ...
            Object.keys(selectedWorkflow.data).forEach((id) => validNodeIds.add(String(id)));
          }
        }
      ...
      retry: (generationId) => {
        const { generations, selectedWorkflow, generate, setWorkflowInput } = get();
        const gen = generations.find((g) => g.id === generationId);
        if (gen && selectedWorkflow && selectedWorkflow.id === gen.workflow_id) {
          // Restore inputs
      ...
      updateWorkflowMeta: async (workflowId, meta) => {
        try {
          ...
          set((state) => {
            ...
            const newSelected =
              state.selectedWorkflow?.id === workflowId ? updatedWorkflow : state.selectedWorkflow;
