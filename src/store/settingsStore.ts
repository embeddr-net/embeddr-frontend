...
const syncLegacyPluginSettings = (pluginSettings: Record<string, Record<string, any>>) => {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.setItem(LEGACY_PLUGIN_SETTINGS_STORAGE_KEY, JSON.stringify(pluginSettings));
...
interface SettingsState {
  ...
  themePackSources: Array<string>;
  setThemePackSources: (sources: Array<string>) => void;
  ...
  setCommandBarHoverParams: (params: { enabled: boolean; delay: number }) => void;
  ...
  setDockConfig: (config: Record<string, { order: number; visible: boolean }>) => void;
  updateDockConfig: (id: string, update: Partial<{ order: number; visible: boolean }>) => void;
...
export const useSettingsStore = create<SettingsState>()(
  persist(
    (set, get) => ({
      ...
      setCommandBarShowDividers: (show) => set({ commandBarShowDividers: show }),
      ...
      setCommandBarHoverParams: (params) => set({ commandBarHoverParams: params }),
