...
export const WebSocketProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const refreshClients = React.useCallback(async () => {
    try {
      ...
      const baseUrl = !root || root.startsWith("/") ? window.location.origin + (root || "") : root;
  ...
  useEffect(() => {
    ...
    const handleVisibility = () => {
      if (document.visibilityState === "visible") {
        ...
        if (!ws || ws.readyState !== WebSocket.OPEN) {
          ...
          console.log("[WebSocketProvider] Tab visible, socket gone — reconnecting");
          shouldReconnectRef.current = true;
          ...
          console.log("[WebSocketProvider] Tab visible, verifying connection...");
          try {
            ws.send("ping");
          } catch {
    ...
    const sendHandler = (payload: any) => {
      if (wsRef.current?.readyState === WebSocket.OPEN) {
        ...
        console.warn("[WebSocketProvider] Cannot send, socket not open", payload);
      }
    };
