...
import { ClockPlus, FolderSyncIcon, ScanEye, Search, Settings2Icon, X } from "lucide-react";
...
import { getArtifactImportSource, getArtifactOrigin, getArtifactProviderId } from "@/lib/providers";
...
const mapArtifactToImage = (a: Artifact, knownProviders?: Set<string>): PromptImage => {
  ...
  return {
    id: a.id, // Cast UUID to any to bypass number type check till PromptImage is updated
    created_at: a.created_at,
    url: contentUrl,
    image_url: contentUrl,
    thumb_url: previewUrl,
    file_size: a.blob_size || 0,
    prompt: a.metadata_json?.prompt || a.metadata_json?.label || a.uri?.split("/").pop() || "",
...
const ExplorePage = () => {
  ...
  const [showSidebar, setShowSidebar] = useLocalStorage("explore-show-sidebar", true);
  ...
  useEffect(() => {
    ...
    const unsubCreated = globalEventBus.on("plugin:artifact.created", handleLiveUpdate);

    const unsubThumbnail = globalEventBus.on("artifact:thumbnail_generated", handleLiveUpdate);
  ...
  const providerSet = useMemo(() => new Set(artifactRegistry?.providers || []), [artifactRegistry]);
  ...
  const {
    ...
    queryFn: async ({ pageParam }) => {
      ...
      if (searchImageId) {
        const res = await embeddrApi.lotus.invoke<SearchResult>("search.similar", {
        ...
        let artifacts = await Promise.all(ids.map((id) => embeddrApi.artifacts.get(String(id))));
        if (selectedTypeName) {
          artifacts = artifacts.filter(
            (a) => a.type_name === selectedTypeName || a.base_type_name === selectedTypeName,
          );
        }
        return {
    ...
    enabled: activeTab === "search" && (activeSearchQuery.length > 0 || searchImageId !== null),
  });
  ...
  const filterByProvider = (items: Array<PromptImage>) => {
  ...
  const originOptions = useMemo(() => artifactRegistry?.origins || [], [artifactRegistry]);
  ...
  const currentFetchNext = activeTab === "search" ? fetchNextSearchPage : fetchNextPage;
  const currentHasNext = activeTab === "search" ? hasNextSearchPage : hasNextPage;
  ...
  useEffect(() => {
    if (currentGallery?.id === "virtual-gallery" && currentPosts.length > 0) {
      ...
      const totalImages = currentHasNext ? lightboxPosts.length + 100 : lightboxPosts.length;
    ...
    if (index !== -1) {
      ...
      openImage(
        embeddrApi.artifacts.getContentUrl(String(image.id)),
        {
          ...
          totalImages: currentHasNext ? lightboxPosts.length + 100 : lightboxPosts.length,
        },
        lightboxIndex,
      );
    }
        ...
        >
          ...
          <div className="flex items-center shrink-0 border border-foreground/10 p-1 bg-card/20 gap-1 rounded-md">
            <Button
              variant="outline"
              size="icon"
              className={cn("border ring-0!", showSidebar && "bg-foreground/20!")}
            ...
            <div className="flex items-center justify-between bg-primary/5 px-1 pl-2 py-1.5 border border-foreground/10 animate-in fade-in slide-in-from-top-1">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium">
                  ...
                  <span className="text-muted-foreground">#{searchImageId}</span>
                </span>
              </div>
              <Button
                variant="ghost"
          ...
          <TabsContent value="search" className="flex-1 m-0 overflow-hidden rounded-md">
            {activeTab === "search" &&
              (isSearchLoading ? (
            ...
            {activeTab === "random" &&
              (isLoading ? (
                ...
                <PostsScrollArea
                  ...
                  onSearchByImage={handleSearchByImage}
                  selectedId={selectedImage?.id}
                  queryKey={["items", "random", selectedLibraryId, selectedCollectionId]}
                  gridCols={autoGrid ? null : gridCols}
                  imageFit={imageFit}
                  useOriginalImages={useOriginalImages}
                />
            ...
            {activeTab === "new" &&
              (isLoading ? (
                ...
                <PostsScrollArea
                  ...
                  onSearchByImage={handleSearchByImage}
                  selectedId={selectedImage?.id}
                  queryKey={["items", "new", selectedLibraryId, selectedCollectionId]}
                  gridCols={autoGrid ? null : gridCols}
                  imageFit={imageFit}
                  useOriginalImages={useOriginalImages}
                />
            ...
            {activeTab === "following" &&
              (isLoading ? (
                ...
                <PostsScrollArea
                  ...
                  onSearchByImage={handleSearchByImage}
                  selectedId={selectedImage?.id}
                  queryKey={["items", "following", selectedLibraryId, selectedCollectionId]}
                  gridCols={autoGrid ? null : gridCols}
                  imageFit={imageFit}
                  useOriginalImages={useOriginalImages}
                />
