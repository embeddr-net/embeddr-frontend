...
import { ScrollArea, Tabs, TabsContent, TabsList, TabsTrigger } from "@embeddr/react-ui/ui";
...
const DebugPage = () => {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden p-1">
      <Tabs defaultValue="server" className="flex-1 flex flex-col min-h-0 gap-1!">
        <div className="p-1 border bg-muted rounded">
          <TabsList className="bg-transparent h-10 w-full justify-start gap-2">
            {tabs.map((tab) => (
              <TabsTrigger key={tab.value} value={tab.value} className={tabsTriggerClasses}>
                {tab.label}
              </TabsTrigger>
            ))}
          </TabsList>
        ...
        {tabs.map((tab) => (
          <TabsContent key={tab.value} value={tab.value} className={tabContentClasses}>
            {tab.component}
          </TabsContent>
        ))}
      </Tabs>
