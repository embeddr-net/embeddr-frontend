...
const PluginPage = ({ pluginId: propPluginId, pageId: propPageId }: PluginPageProps) => {
  ...
  const pageComponent = useMemo(() => {
    ...
    if (pageId) {
      return pages.find((c) => c.id === pageId || (c as any).exportName === pageId);
    }
    return pages[0];
  }, [plugin, pageId]);
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center space-y-2">
          ...
          <p className="text-muted-foreground">The plugin "{pluginId}" could not be found.</p>
        </div>
      </div>
    );
  }
...
class PluginPageErrorBoundary extends React.Component<
  ...
  { error?: Error }
> {
  constructor(props: { pluginName: string; pageId?: string; children: React.ReactNode }) {
    super(props);
        ...
        <div className="flex items-center justify-center h-full ">
          <div className="text-center space-y-2">
            ...
            <p className="text-muted-foreground">
              ...
              {this.props.pageId ? ` (${this.props.pageId})` : ""} failed to render.
            </p>
